...
fig = plt.figure()
fig.suptitle("Subject1: STFT(Actual)", fontsize=20)
...
for i in range(steps):
  ...
  stft_start:int = FS * (3 + WAIT_SECOND_ACTUAL[i] - 1) - 548
  ...
  window = signal.windows.hamming(500)
  nfft = 150
  n = len(Cz_filtered)
  t = n / FS

  f_Cz, t_Cz, Sxx_Cz = signal.stft(Cz_filtered[stft_start:stft_end], FS, window=window, nperseg=500)
  f_C3, t_C3, Sxx_C3 = signal.stft(C3_filtered[stft_start:stft_end], FS, window=window, nperseg=500)
  f_C4, t_C4, Sxx_C4 = signal.stft(C4_filtered[stft_start:stft_end], FS, window=window, nperseg=500)
  f_Fz, t_Fz, Sxx_Fz = signal.stft(Fz_filtered[stft_start:stft_end], FS, window=window, nperseg=500)
  f_F3, t_F3, Sxx_F3 = signal.stft(F3_filtered[stft_start:stft_end], FS, window=window, nperseg=500)
  f_F4, t_F4, Sxx_F4 = signal.stft(F4_filtered[stft_start:stft_end], FS, window=window, nperseg=500)
...
Cz_Sxx_ave = Cz_Sxx_sum/20
C3_Sxx_ave = C3_Sxx_sum/20
C4_Sxx_ave = C4_Sxx_sum/20
Fz_Sxx_ave = Fz_Sxx_sum/20
F3_Sxx_ave = F3_Sxx_sum/20
F4_Sxx_ave = F4_Sxx_sum/20
...
ax1.pcolormesh(Cz_t_sum/20, Cz_f_sum/20, Cz_Sxx_ave, cmap='jet')
ax2.pcolormesh(C3_t_sum/20, C3_f_sum/20, C3_Sxx_ave, cmap='jet')
ax3.pcolormesh(C4_t_sum/20, C4_f_sum/20, C4_Sxx_ave, cmap='jet')
ax4.pcolormesh(Fz_t_sum/20, Fz_f_sum/20, Fz_Sxx_ave, cmap='jet')
ax5.pcolormesh(F3_t_sum/20, F3_f_sum/20, F3_Sxx_ave, cmap='jet')
ax6.pcolormesh(F4_t_sum/20, F4_f_sum/20, F4_Sxx_ave, cmap='jet')
